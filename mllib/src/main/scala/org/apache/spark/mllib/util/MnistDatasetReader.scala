...
case class MnistItem(label: Int, data: Array[Int]) {
  def binaryVector: SV = {
    new SDV(data.map { i =>
      if (i > 30) {
        1D
...
class MnistDatasetReader(labelsFile: String, imagesFile: String)
  extends java.util.Iterator[MnistItem] with Closeable with Logging {
  ...
  logInfo("Labels magic=$magic count= $labelCount")
  ...
  logInfo("Images magic=$magic count=$imageCount rows=$rows cols=$cols")
  assert(imageCount == labelCount)

  var current = 0

  override def next(): MnistItem = {
    try {
